...
if valuee[0] == "1":
    ...
    output = subprocess.getoutput("sudo " + cmd + " --kubeconfig /root/k8s_workspace/admin.conf")
    print(output)
elif valuee[0] == "2":
    pod_name = valuee[2]
    image_name = valuee[1]
    ...
    cmd = "sudo kubectl get pods --kubeconfig /root/k8s_workspace/admin.conf"
    output = subprocess.getoutput(cmd)
    print(output)
elif valuee[0] == "8":
    cmd = "kubectl get deployments --kubeconfig /root/k8s_workspace/admin.conf"
    output = subprocess.getoutput("sudo " + cmd)
    print(output)
elif valuee[0] == "9":
    cmd = "kubectl get svc --kubeconfig /root/k8s_workspace/admin.conf"
    output = subprocess.getoutput(cmd)
    print(output)
elif valuee[0] == "10":
    cmd = "kubectl delete all --all --kubeconfig /root/k8s_workspace/admin.conf"
    output = subprocess.getoutput("sudo " + cmd)
    print(output)
elif valuee[0] == "404":
